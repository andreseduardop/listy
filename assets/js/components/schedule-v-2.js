 ...
 * - The first time a user adds an activity, create the END item with time equal to
 *   the activity time plus 60 minutes.
 * - The END item is editable and non-draggable, placed after all activities and
 *   before the "new activity" row.
 * - The END item has NO actions container (no Save/Discard/Delete links).
 * - New activities are always inserted before END.
 *
 ...
 * ================================ */
/**
 * Adds minutes to an "HH:MM" string and returns a wrapped "HH:MM".
 * @param {string} hhmm
 * @param {number} minutes
 * @return {string}
 */
function addMinutes(hhmm, minutes) {
  // Comentario: convierte HH:MM a minutos totales, suma y envuelve en 24h
  const m = /^\s*(\d{2}):(\d{2})\s*$/.exec(hhmm || "");
  const h = m ? Number(m[1]) : 0;
  const min = m ? Number(m[2]) : 0;
  const total = (h * 60 + min + (minutes || 0) + 24 * 60) % (24 * 60);
...
class Model extends EventTarget {
  ...
  /**
   * Reads items from storage, normalizes shape, and ensures END—if present—is last.
   * @return {!Array<SheduleItem>}
   */
  getAll() {
    // Comentario: lee desde storage y normaliza
    ...
    for (const it of arr) {
      ...
      const time =
        typeof it.time === "string" && /^\d{2}:\d{2}$/.test(it.time)
          ...
          : "08:30";
      if (it.id === END_ID) {
        // Comentario: retiene END si existiera
        endItem = { id: END_ID, text: text || "End", time: time || "23:59" };
      } else {
        normal.push({ id: it.id, text, time });
      }
    }
    // Comentario: retorna con END al final solo si existe
  ...
  /**
   * Adds a new activity; if it's the first one, also creates END at +60min.
   * @param {string} text
   * @param {string=} time
   * @return {void}
   */
  add(text, time = "08:30") {
    ...
    if (activities.length === 0 && !hasEnd) {
      // Comentario: primera actividad → crea END con +60m de la hora de la primera
      const endTime = addMinutes(time, 60);
      const end = /** @type {SheduleItem} */ ({ id: END_ID, text: "End", time: endTime });
      next = [newItem, end];
    } else {
      // Comentario: inserta antes del END (o al final si no existe legacy)
      if (hasEnd) {
  ...
  /**
   * Updates the text of an item; removes it if the text becomes empty (except END).
   * @param {string} id
   * @param {string} text
   * @return {void}
   */
  updateText(id, text) {
    // Comentario: actualiza el texto; en END, obliga texto por defecto si queda vacío
    const t = String(text ?? "").trim();
    ...
    if (id !== END_ID && !t) {
      // Comentario: elimina el item estándar si quedó vacío
  ...
  /**
   * Updates the time of an item (END inclusive).
   * @param {string} id
   * @param {string} time
   * @return {void}
   */
  updateTime(id, time) {
    // Comentario: valida HH:MM y aplica
  ...
  /**
   * Removes an item by id (ignored for END).
   * @param {string} id
   * @return {void}
   */
  remove(id) {
    // Comentario: no elimina END
    if (id === END_ID) return;
  ...
  /**
   * Moves an item to a target index in the same list (END is not movable).
   * @param {string} id
   * @param {number} toIndex
   * @return {void}
   */
  moveToIndex(id, toIndex) {
    // Comentario: no mueve END
    if (id === END_ID) return;

    const items = this.getAll();
    const activities = items.filter((i) => i.id !== END_ID);
    const end = items.find((i) => i.id === END_ID) || null;

    if (activities.length <= 1) return;

    const from = activities.findIndex((i) => i.id === id);
    if (from === -1) return;

    // Comentario: clamp destino dentro de actividades
    let dest = Math.max(0, Math.min(activities.length, Number(toIndex)));
